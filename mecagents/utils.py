...
def save_config_to_json(
    config_dict: Dict[str, Any], output_path: str, filename: str = "config.json"
):
...
def save_training_results(
    results: Dict[str, Any], output_path: str, filename: str = "training_results.json"
):
...
def calculate_model_parameters(model) -> Dict[str, int]:
    ...
    return {
        ...
        "trainable_percentage": (
            round(trainable_params / total_params * 100, 2) if total_params > 0 else 0
        ),
    }
...
def validate_dataset_format(dataset) -> Dict[str, Any]:
    ...
    validation_results = {
        ...
        "avg_prompt_length": sum(len(str(s.get("prompt", ""))) for s in dataset)
        / len(dataset),
        "avg_code_length": sum(len(str(s.get("cadquery", ""))) for s in dataset)
        / len(dataset),
    }
...
def compare_model_outputs(
    prediction: str, ground_truth: str, metrics: List[str] = None
) -> Dict[str, Any]:
