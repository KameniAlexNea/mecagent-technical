...
class DataProcessor:
    ...
    def convert_dataset_to_conversations(
        ...
        converted_dataset = [
            self.convert_to_conversation_format(sample) for sample in dataset
        ]

        print(f"Converted {len(converted_dataset)} samples")
        return converted_dataset

    def create_inference_messages(
        self, image: Any, instruction: Optional[str] = None
    ) -> List[Dict[str, Any]]:
    ...
    def get_sample_data(
        self, index: int = 0, from_test: bool = False
    ) -> Dict[str, Any]:
        """Get a sample from the dataset"""
        dataset = (
            self.test_dataset if from_test and self.test_dataset else self.train_dataset
        )
        ...
        if index >= len(dataset):
            raise ValueError(
                f"Index {index} out of range for dataset of size {len(dataset)}"
            )
