...
class TrainingManager:
    ...
    def create_trainer(
        self, model: Any, processor: Any, train_dataset: List[Dict[str, Any]]
    ) -> SFTTrainer:
    ...
    def _create_training_summary(
        ...
        gpu_stats = (
            torch.cuda.get_device_properties(0) if torch.cuda.is_available() else None
        )
        max_memory = (
            round(gpu_stats.total_memory / 1024 / 1024 / 1024, 3) if gpu_stats else 0
        )

        memory_used_for_training = round(end_memory - start_memory, 3)

        summary = {
            "training_time_seconds": self.training_stats.metrics.get(
                "train_runtime", 0
            ),
            "training_time_minutes": round(
                self.training_stats.metrics.get("train_runtime", 0) / 60, 2
            ),
            ...
            "memory_percentage": (
                round(end_memory / max_memory * 100, 3) if max_memory > 0 else 0
            ),
            "training_memory_percentage": (
                round(memory_used_for_training / max_memory * 100, 3)
                if max_memory > 0
                else 0
            ),
            ...
            summary.update(
                {
                    ...
                    "train_samples_per_second": self.training_stats.metrics.get(
                        "train_samples_per_second"
                    ),
                    "train_steps_per_second": self.training_stats.metrics.get(
    ...
    def _print_training_summary(self, summary: Dict[str, Any]):
        ...
        print(
            f"Memory used for training: {summary.get('memory_used_for_training_gb', 0)} GB"
        )
